...
export default function Register() {
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      alert("Pendaftaran berhasil. Silakan masuk ke akun Kamu.");
      navigate('/login');
    } catch (error) {
      console.error(error);
      alert("Pendaftaran gagal. Silakan periksa kembali data Kamu.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex-1 lg:w-1/2 flex items-center justify-center p-6 bg-slate-50">
        <div className="w-full max-w-[320px] lg:max-w-[420px] bg-white p-8 lg:p-12 rounded border border-slate-200 lg:border-none lg:bg-transparent">
          ...
          <div className="mb-8 text-center lg:text-left">
            <h3 className="text-2xl lg:text-3xl font-bold text-slate-900 tracking-tight">Buat Akun</h3>
            <p className="text-slate-500 mt-1 text-xs lg:text-sm font-medium">Mulai langkah awal menuju finansial yang lebih tertata.</p>
          </div>

          <div className="space-y-4 lg:space-y-4">
            <div className="space-y-1.5">
              <label className="text-xs lg:text-sm font-bold text-slate-600 ml-0.5">Nama</label>
              <input 
                type="text" 
                placeholder="Masukkan nama Kamu" 
                value={name} 
                onChange={(e) => setName(e.target.value)} 
                className="w-full p-3 lg:p-4 bg-white border border-slate-200 rounded outline-none focus:border-slate-900 text-xs lg:text-sm font-semibold transition-colors" 
              />
            </div>
          ...
          <div className="mt-8 flex items-center justify-between">
            ...
            <span className="px-3 text-[10px] text-slate-400 font-bold whitespace-nowrap tracking-widest uppercase">Atau</span>
            <hr className="w-full border-slate-200" />
          </div>
          ...
          <div className="mt-10 text-center text-xs lg:text-sm">
            ...
            <Link to="/login" className="text-slate-900 font-bold hover:underline underline-offset-4 decoration-2 transition-all">
              Masuk di sini
            </Link>
          </div>
        </div>
