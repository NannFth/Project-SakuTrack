...
const RekomendasiKeuangan = () => {
    ...
    const fetchRecommendations = async () => {
      try {
        ...
        if (data.success && data.data.length > 0) {
          ...
          let extraIndex = 0;
          while (formattedData.length < 4 && extraIndex < defaultExtras.length) {
  ...
  const getRouteByType = (type) => {
    switch (type) {
      case 'daily_limit': 
      case 'setup_limit':
      case 'financial_health': return '/proyeksi-keuangan';
      case 'warning_wants': 
      case 'yesterday_eval': 
      case 'streak_praise': 
      case 'input_expense': return '/kelola-transaksi';
      case 'goal_progress': 
      case 'surplus_allocation': 
      case 'check_goals': return '/target-tabungan';
      case 'challenge': return '/kelola-transaksi'; 
  ...
  const getThemeByType = (type) => {
    switch (type) {
      case 'daily_limit': 
        return { bg: "bg-white", border: "border-blue-500", text: "text-slate-900", iconBg: "bg-blue-50", iconText: "text-blue-600", button: "bg-blue-600 text-white hover:bg-blue-700" };
      case 'warning_wants': 
        return { bg: "bg-white", border: "border-rose-500", text: "text-slate-900", iconBg: "bg-rose-50", iconText: "text-rose-600", button: "bg-rose-600 text-white hover:bg-rose-700" };
      case 'goal_progress': 
        return { bg: "bg-white", border: "border-emerald-500", text: "text-slate-900", iconBg: "bg-emerald-50", iconText: "text-emerald-600", button: "bg-emerald-600 text-white hover:bg-emerald-700" };
      default: 
        return { bg: "bg-white", border: "border-slate-200", text: "text-slate-900", iconBg: "bg-slate-50", iconText: "text-slate-500", button: "bg-slate-100 text-slate-800 hover:bg-slate-200" };
    }
  };
  ...
  const supportingCards = recommendations.slice(1, 4);
    ...
    <div className="max-w-6xl mx-auto p-4 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {supportingCards.map((item) => (
          <div key={item.id} className="bg-white p-6 rounded border border-slate-200 shadow-sm flex flex-col">
            <div className="flex-1">
              <div className={`w-12 h-12 rounded flex items-center justify-center mb-5 ${item.theme.iconBg} ${item.theme.iconText}`}>
